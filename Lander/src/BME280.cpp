...
int BME280::start(bme280_oversampling temperature, bme280_oversampling pressure, bme280_oversampling humidity) {
    if (read8(address, BME280_CHIP_ID_REG) != 0x60) {
        return 1;
    }
    readCoefficients();
    //page 25
    write8(address, BME280_CONTRL_HUMD_REG, humidity);
    write8(address, BME280_CONTRL_REG, (uint8_t) (3 | temperature << 5 | pressure << 2));
    return 0;
}
...
int BME280::read_unprocessed(int32_t *temperature, int32_t *pressure, int32_t *humditiy) {
    uint32_t buffer[8];
    readBuffer(address, 0xF7, buffer, 8);
    *pressure = (buffer[0] << 16) | (buffer[1] << 8) | buffer[2];
    *temperature = (buffer[3] << 16) | (buffer[4] << 8) | buffer[5];
    *humditiy = (buffer[6] << 16) | (buffer[7] << 8) | buffer[8];
    return 0;
}
